...
public class MockStream {
    ...
    public static void main(String[] args) throws JsonProcessingException {
        ...
        // 需求：过滤出年龄大于40的歌手的名字
        MyList<String> result = personMyList.filter(person -> person.getAge() > 40).map(person -> person.getName());
        ...
        List<String> collect = list.stream().filter(person -> person.getAge() > 40).map(person -> person.getName()).collect(Collectors.toList());
        prettyPrint(collect);
    }
